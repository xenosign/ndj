...
type PageData = {
  ...
  participantProfiles: { id: string; nickname: string | null; avatar_url: string | null }[];
  characterMap: Record<string, string | null>;
  reactionMap: Record<string, string>;
  reactionTimeMap: Record<string, string>;
  todayMission: { id: string; content: string; photo_url: string | null } | null;
  tomorrowMission: { id: string; content: string } | null;
  missionPhotoSignedUrl: string | null;
  verificationCount: number;
  sentimentParticipants: Record<'good' | 'neutral' | 'bad', { nickname: string | null; avatarUrl: string | null; reaction: string }[]>;
  daysLeft: number;
  startWeight: number;
  targetWeight: number;
  progress: number;
        ...
        <div className="px-4 py-5 flex flex-col gap-4 pb-8">
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              {bar('w-3/4')}
              {bar('w-1/2')}
              <div className={`${pulse} h-10 w-full rounded-xl`} style={{ backgroundColor: '#D4C0F0' }} />
            </div>
          </div>

          {/* EnemyReactionBar + ReactionSummaryCard 스켈레톤 */}
          <div className="flex flex-col gap-2">
            {bar('w-24 h-4 ml-1')}
            <div
            ...
            <div
              className="rounded-2xl px-5 py-4 grid grid-cols-3 gap-3"
              style={{ backgroundColor: '#F8F4FF', boxShadow: '0 4px 20px rgba(123,77,190,0.28)' }}
            >
              {[...Array(3)].map((_, i) => (
                <div key={i} className={`${pulse} h-14 rounded-xl`} style={{ backgroundColor: '#D4C0F0' }} />
              ))}
            </div>
          </div>
...
export default function MyDietPageClient({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      const todayWeight: number | null = latestLog?.logged_date === today ? (latestLog.weight as number) : null;
      const todayPhotoPath: string | null = latestLog?.logged_date === today ? (latestLog.photo_url as string | null) : null;
      ...
      setData({
        ...
        todayWeight,
        todayPhotoPath,
        participantProfiles: (participantProfiles ?? []) as PageData['participantProfiles'],
        characterMap,
        reactionMap,
        reactionTimeMap,
  ...
  const {
    challenge, inviteCode, recentLogs, currentWeight, todayWeight, todayPhotoPath,
    participantProfiles, characterMap, reactionMap, reactionTimeMap,
    todayMission, tomorrowMission, missionPhotoSignedUrl, verificationCount,
    sentimentParticipants, daysLeft, startWeight, targetWeight, progress, today, tomorrow,
  } = data;
      ...
      <ScrollableArea>
        <div className="px-4 py-5 flex flex-col gap-4 pb-8">
          ...
          <div className="flex flex-col gap-2">
            ...
            <CommentCard challengeId={challenge.id} challengeOwnerId={userId} />
          </div>
